      ...
      {loading ? <div className="flex justify-center py-16"><div className="spinner" /></div>
       : lists.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {lists.map(l => (
            <div key={l.id} className="card p-5 group">
              <div className="flex items-start justify-between mb-4">
                <div className="w-11 h-11 rounded-xl bg-emerald-50 flex items-center justify-center">
                  <Users className="w-5 h-5 text-emerald-600" />
                </div>
                <button onClick={() => del(l.id)} className="p-2 rounded-lg text-red-400 opacity-0 group-hover:opacity-100 hover:bg-red-50 transition-all">
